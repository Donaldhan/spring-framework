 ...
 *单例目标源SingletonTargetSource为TargetSource的实现，持有一个目标对象。
 *此类为TargetSource的默认实现，被Spring AOP 框架使用。通常情况下，不需要在应用代码中创建
 *此类的对象。
 ...
 *此类可以被序列化。然而SingletonTargetSource实际的序列化依赖于目标是否可以被序列化。
 * @author Rod Johnson
